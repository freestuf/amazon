...
function SADDB() {
  ...
  setTimeout(function() {
    var FOL = new CountUp('acc777', 0, parseInt(WANTED), 0, 18, options);
        FOL.start(function() {
          $("#step34").html("Step 4: Verify that you are not a robot")
          $("#CONS").fadeOut(1500, function() {
$("#CONS").html("<strong class='reddd'>Mistake! Verify that you are a human</strong> Your coupon code is : AMAZON***** Click the Verification button to show $" + WANTED + " Amazon Coupon");
            $("#CONS").fadeIn(1500);
...
function SADD() {
  ...
  $("#step34").html("Step 3: Amazon Coupon being generated")
  ...
  setTimeout(function() {
    var FOL = new CountUp('acc', 0, parseInt(WANTED), 0, 18, options);
        ...
        setTimeout(function() {
          $("#CONS").html("Adding <strong>" + WANTED + "</strong> Amazon Coupon...")
        }, 9000)

        setTimeout(function() {
          $("#CONS").html("Disconnecting from <strong>Monopoly Go servers</strong>...")
        }, 14000)
  }, 1500)
...
function S1() {
  ...
  if (WANTED == "" || TUSER == "") {
    alert("Enter your user name and the amount of resources, then click Continue")
  } else {
